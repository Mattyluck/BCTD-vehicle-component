...
func Test1() {
	...
	txsCount := []uint{1}
	txSize := []uint{300032} //报文的大小

	for ind := 0; ind < len(txSize); ind++ {
		size := txSize[ind]
		var txBody string
		...
		for k := 0; k < 51; k++ {
			for i := 0; i < len(txsCount); i++ {
				...
				fmt.Printf("块高度为:%d, 一个区块内的交易数：%d(笔), 交易大小：%d(Byte),证明总大小：%d (Byte), 生成证明的时间为: %d ms, 验证时间为：%d ms\n",
					k, txs, size, proofLen, proofTime, verifyTime)
			}
		}
	}
}
